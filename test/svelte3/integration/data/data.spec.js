...
describe('SvelteDoc v3 - Props', () => {
    ...
    it.only('Keywords should be included', (done) => {
        parser.parse({
            version: 3,
            filename: path.resolve(__dirname, 'data.keywords.svelte'),
            features: ['data', 'keywords'],
            includeSourceLocations: true,
            ignoredVisibilities: []
        }).then((doc) => {
            expect(doc, 'Document').to.exist;
            expect(doc.data, 'Document data').to.exist;
            expect(doc.data, 'Document data length').to.have.length(1);
            expect(doc.data[0].keywords, 'Document data keywords').to.exist;
